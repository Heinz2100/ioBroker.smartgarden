 ...
 * SVN:                 $Rev: 3207 $ ($Date: 2024-05-25 11:37:56 +0200 (Sa, 25 Mai 2024) $)
 * contains some functions available at forum.iobroker.net, see function header
 *
 * Modifikation v2-Patch 1.0.0 (2026-07-30):
 *  - Umstellung auf GARDENA smart system API v2 (URL_COMMAND/URL_LOCATIONS/URL_WEBSOCKET)
 *  - mowerIsMowing() um v2 MowerActivity-Codes erweitert
 *  - aeltere Aenderungen: siehe Projekthistorie
 */
"use strict";

// some variables  
const apirev = '$Rev: 3207/v2 $'; // modifiziert: GARDENA smart system API v2 (Spec 2.3.6)
...
const minGardenaAuthtokenTimeout = 60; // in seconds

const UNDEFINED = 'undefined';

const URL_OAUTH2_TOKEN = '/v1/oauth2/token';
const URL_COMMAND = '/v2/command/';
const URL_LOCATIONS = '/v2/locations';
const URL_WEBSOCKET = '/v2/websocket';
...
let websocketclient_refresh_running = false;
let client = null;
let gardena_smart_host;
let gardena_api_key;
let gardena_application_secret;
...
let configGardenaAuthtokenFactor = 1.0;
let configBeautifyLog = true;
...
function setGardenaLocationNotExists(lid) {
	if (LOCATIONS.hasOwnProperty('LOCATION_' + lid) === false) {
		ju.adapterloginfo(3, 'setGardenaLocationNotExists: new location added: ' + beautifyStateId('LOCATION_' + lid));
...
function setGardenaDeviceNotExists(lid, did) {
	...
	if (LOCATIONS['LOCATION_' + lid].hasOwnProperty('DEVICE_' + did) === false) {
		...
		ju.adapterloginfo(3, 'setGardenaDeviceNotExists: new device added: ' + beautifyStateId('LOCATION_' + lid) + '.' +  beautifyStateId('DEVICE_' + did));
...
function setGardenaServiceNotExists(lid, did, sid, stype) {
	...
	if (LOCATIONS['LOCATION_' + lid]['DEVICE_' + did].hasOwnProperty('SERVICE_' + stype + '_' + sid) === false) {
		ju.adapterloginfo(3, 'setGardenaServiceNotExists: new service added: ' + beautifyStateId('LOCATION_' + lid) + '.' + beautifyStateId('DEVICE_' + did) + '.' + beautifyStateId('SERVICE_' + stype + '_' + sid));
...
function setGardenaAttribute(s, value) {
	...
	if (arr.length === 0) {
		ju.adapterloginfo(1, 'setGardenaAttribute: empty id: ' + beautifyStateId(s));
		return;
	}
	...
	if (loc.search('LOCATION_') === -1) {
		ju.adapterloginfo(1, 'setGardenaAttribute: no location found: ' + beautifyStateId(s));
		return;
	} else {
...
function setLeftOverTimer(tim) {
	...
	adapter.getState(sgEncode(mytim), function(err, state) {
		if (!err && state.val !== 'null' && state.val > 1) { 
			difference = state.val - 1;
			sgSetState(mytim, difference.toString()); // duration_leftover_i is a string // make a string 
			if (difference > 1) {
 ...
 * @param   {string}  service_log, string for loggingm e.g. 'Power Socket'
 * @param   {string}  serviceid  id for service
 * @param   {string}  service_command the command, eg. 'START_SECONDS_TO_OVERRIDE'
 * @param   {string}  service_control  control for device, eg. 'MOWER_CONTROL'
 * @param   {string} or {integer} value eg. 60 or 'PAUSE_60'
 */
function putRequest (service_log, serviceid, service_command, service_control, value) {
	let options = {
		...
		headers: {
			...
			// not required since API v1.1.0  'Authorization-Provider': 'husqvarna',
	...
	if (value !== undefined) {
		options.json.data.attributes.seconds = value;
	}

	request(options, function(err, response, body) {
		let e = null;
		
		ju.adapterloginfo(3, 'request ' + service_log + ' ...');

		if (err || !response || response.statusCode >= 300) {
			// failure
			ju.adapterloginfo(1, service_log + ' command failure, options.url=' + options.url);

			if (err) {
				e = err;
				ju.adapterloginfo(1, service_log + ' command failure: return with error');
			}
			if (response) {
				ju.adapterloginfo(1, service_log + ' command failure.response.statusCode/Message=' + response.statusCode + '/' + response.statusMessage);
				e = new Error(service_log + ' command failure ' + response.statusCode+ ' ' + response.statusMessage);
			} else {
				ju.adapterloginfo(1, service_log + ' command failure. No response');
				e = new Error(service_log + ' command failure. No response');
			}
			adapter.log.error(e);
		} else {
			// successful
			ju.adapterloginfo(1, service_log + ' command successful. options.url=' + options.url + ' response.statusCode/Message=' + response.statusCode + '/' + response.statusMessage);
		}
	})
}

/**
 * Check that all values are multiples of 60
 * if value not a number then set value to 60, negative values are interpreted as positive values
 * @param  value
 */
...
/**
 * Send command to Gardena smart system
 * supported services: SERVICE_POWER_SOCKET, ...
 * @param   {string}  id (encoded)
 * @param   {object}  state
 */
						...
						case 'duration_value':
							...
							switch (state.val) {
								...
								default:
									...
									value = checkValue60(value);
									break;
							}

							incLimitCounter(limitCounter);
							putRequest ('Power Socket', serviceid, service_command, service_control, value);
							break;
						default:
			...
			case 'SERVICE_MOWER':
				{
					switch (status) {
						case 'activity_control_i':
							...
							incLimitCounter(limitCounter);
							putRequest ('Mower', serviceid, service_command, service_control, value);
							break;
						default:
							setSaveState(id, state.val);
							break;
									...
									case 'STOP_UNTIL_NEXT_TASK':
										{
											...
											incLimitCounter(limitCounter);
											putRequest ('Valve Set', serviceid, service_command, service_control);
						...
						case 'duration_value':
							{
								...
								incLimitCounter(limitCounter);
								putRequest ('Valve', serviceid, service_command, service_control, value);
								break;
							}
						default:
...
/**
 * Setzt den Wert für einen gegebenen Status 
 * @param   {string}  s    state, not encoded
 * @param   {object}  v    value to set
 * @param   {boolean} ack  set ack
 */
function sgSetStateAck(s, v, ack) {
	//s=smartgarden.0.LOCATION_12345678-cd2a-9876-759a-b16c124347cf.DEVICE_abc12345-92c1-417a-1205-f6e6a3e5127e.SERVICE_COMMON_abc12345-92c1-417a-1205-f6e6a3e5127e.rfLinkState_value
	let bs = beautifyStateId(s); // s should be encoded, but it's just for log, so for performance we will not do that
...
/**
 * beautifying the state id / shortening state id if configBeautifyLog=true
 * just for better reading
 * @param   {string}  s    state id
 * @return  {string}  v    beautified state id
 */
exports.beautifyStateIdName = function(s) {
	return beautifyStateId(s);
}
...
function beautifyStateId(s) {
	...
	// smartgarden.
	// 0.
	// LOCATION_12345678-cd2a-9876-759a-b16c124347cf.
	// DEVICE_abc12345-92c1-417a-1205-f6e6a3e5127e.
	// SERVICE_COMMON_abc12345-92c1-417a-1205-f6e6a3e5127e.
	// rfLinkState_value
	// 
	// sg.
	// 0.
...
function reconnect() {
	ju.adapterloginfo(1, 'reconnect!');

	if (gardena_refresh_token !== undefined && gardena_refresh_token !== '' && gardena_refresh_token !== null) {
...
function reconnectWithRefreshToken() {
	...
	websocketclient_refresh_running = true;
	...
	if (gardena_refresh_token === undefined || gardena_refresh_token === '' || gardena_refresh_token === null) {
		...
		websocketclient_refresh_running = false;
	} else {
		...
		request(options_refresh, function(err, response, body) {
			if (err || !response || response.statusCode >= 300) {
				...
				if (response.body.hasOwnProperty('refresh_token')) {
				...
				timeout = Math.round(response.body.expires_in * configGardenaAuthtokenFactor);
				if (timeout < minGardenaAuthtokenTimeout) {
				...
				refresh_timer = setTimeout(reconnect, timeout * 1000);
				ju.adapterloginfo(1, 'reconnectWithRefreshToken: expires_in/timeout/factor:' + response.body.expires_in + ' / ' + timeout + ' / ' + configGardenaAuthtokenFactor);
				...
				get_websocket(function(err, websocket) {
					if (err) {
						...
						refresh_timer = setTimeout(reconnect, configConnectionRetryInterval * 1000);
					} else {
...
function mowerIsMowing(val) {
	...
	switch (val) {
		...
		// neue Codes der GARDENA smart system API v2 - Maeher ist nicht auf dem Rasen:
		case 'PAUSED_IN_CS': // - The mower is paused in charging station.
		case 'PARKED_FROST': // - The mower is parked due to frost.
		case 'PARKED_NO_LIGHT': // - The mower is parked due to insufficient light conditions.
		case 'PARKED_MOWING_COMPLETED': // - The mower is parked because all areas have been fully mowed.
		case 'PARKED_RAIN': // - The mower is parked due to rain.
		...
		// neue Codes der GARDENA smart system API v2 - Maeher befindet sich im Garten,
		// daher konservativ wie MOWING behandeln (relevant fuer Bewaesserungs-Verriegelung):
		case 'STOPPED_IN_GARDEN': // - The mower is stopped in the garden.
		case 'INITIATE_NEXT_ACTION': // - The mower is initiating the next action.
		case 'SEARCHING_FOR_SATELLITES': // - The mower is searching for satellites.
...
function prepLeftoverTimer(activitytimer, name, a, type, propval) {
	...
	if (a === '.activity_value') {
		...
		if (propval !== 'TIME_LIMITED_ON') {
			...
			setStateNotExists(  nx1, 
								...
								getPreDefineStateAttribute(type, x1.substr(1), 'unit'),
								true,
	...
	if (activitytimer.valid === true && activitytimer.duration !== 'null' && activitytimer.timestamp !== 'null') {
		...
		// but in January 2023 the device was started in the future :-)
		// so we're using the minimum duration
		let curDate = new Date();
		let curDate_ms = curDate.getTime();
		let startDate = new Date(activitytimer.timestamp);
		let startDate_ms = startDate.getTime();
		let endDate_ms = startDate_ms + (activitytimer.duration * 1000);
		
		let diff = endDate_ms - curDate_ms;
		
		let difference = Math.min(diff, (activitytimer.duration * 1000));
		difference = Math.round(difference / 1000 / 60);


		if (startDate_ms > curDate_ms) {
			ju.adapterloginfo(2, 'prepLeftoverTimer: startDate > curDate: ' + startDate.toUTCString() + '>' + curDate.toUTCString());

			ju.adapterloginfo(3, 'prepLeftoverTimer: startDate_ms: ' + startDate_ms);
			ju.adapterloginfo(3, 'prepLeftoverTimer: activitytimer.duration: ' + activitytimer.duration);

			ju.adapterloginfo(3, 'prepLeftoverTimer: curDate_ms: ' + curDate_ms);
			ju.adapterloginfo(3, 'prepLeftoverTimer: endDate_ms: ' + endDate_ms);
			ju.adapterloginfo(3, 'prepLeftoverTimer: round((endDate_ms - curDate_ms)/1000): ' + Math.round(diff/1000));
			
			ju.adapterloginfo(3, 'prepLeftoverTimer: round(difference/1000/60): ' + difference);
		}
		...
		setStateNotExists(  nx, 
			difference.toString(),  // make a string 
			'string',   // has to be a string
			'text',
			'',
			true,
...
/**
 * Setzt die Werte für die States eines Services einer Message vom Gardena Smart System
		...
		if (propattributes === 'attributes') {
			for (let prop2 in m[propattributes]) {
				for (let prop3 in m[propattributes][prop2]) {
					...
					setStateNotExists(  n, 
										ma_val, 
										getPreDefineStateAttribute(m.type, a.substr(1), 'type'), 
										getPreDefineStateAttribute(m.type, a.substr(1), 'role'),
										getPreDefineStateAttribute(m.type, a.substr(1), 'unit'),
										true,
										getPreDefineStateAttribute(m.type, a.substr(1), 'write'),
...
function setStateNotExists(id, val, stype, srole, sunit, sread, swrite, snative) {
	...
	adapter.setObjectNotExists(sgEncode(id), {
						...
						if (err) {
							ju.adapterloginfo(1, 'ERROR setStateNotExists: ' + beautifyStateId(id) + ' / ' + val );
						} else {
							sgSetState(id, val);
						}
					});
...
function setGardenaStateNotExists(id, val, stype, srole, sunit, sread, swrite, sdef, snative) {
	...
	adapter.setObjectNotExists(sgEncode(id), {
						...
						if (err) {
							ju.adapterloginfo(1, 'ERROR setGardenaStateNotExists: ' + beautifyStateId(id) + ' / ' + val );
						} else {
...
function getPreDefineStateAttribute(servicetype, name, attr) {
	...
	if (dp.hasOwnProperty(service)) {
		if (dp[service].hasOwnProperty(name)) {
			...
			ju.adapterloginfo(1, 'getPreDefineStateAttribute: unknown dp name: ' + name + ' in service: ' + service);
		}
	} else {
		ju.adapterloginfo(1, 'getPreDefineStateAttribute: unknown service: ' + service + ' name of dp: ' + name);
	}
...
*/
	...
	if (commontype === 'string') {
		return val.toString(); // make a string
	}
...
class Client {
	...
	on_close(close) {
		...
		if (websocketclient_refresh_running == false) 
		{
			t = 'on_close: trying to reconnect'
			ju.consolelog(1, '### ' + ju.curTime() + " " + t);
			ju.adapterloginfo(1, t);
			reconnect();
		}
...
exports.setAdapter = function(adapter_in) {
	...
	configGardenaAuthtokenFactor = adapter.config.gardena_authtoken_factor;
	configBeautifyLog = adapter.config.beautifyLog; 
 ...
 * 
 * @param   {string}  mainrev  Revision von Main im Format $Rev: 3207 $
 */
...
/**
 * Connect to Gardena smart system 
 */
function connect(callback) {
    ...
    gardena_api_key = adapter.config.gardena_api_key.trim();
	gardena_application_secret = adapter.config.gardena_application_secret.trim();
	...
	if (gardena_application_secret.length === 0) {
		ju.adapterloginfo(1, 'connect ... empty application secret');
		ju.consolelog(1, 'connect ... empty application secret');
	}
	//ju.adapterloginfo(1, "Gardena Smart System Service hosts at: smart_host: " + gardena_smart_host + " authentication_host: " + gardena_authentication_host + ".");
	//ju.adapterloginfo(1, "Gardena Smart System Service credentials/1: user: " + gardena_username + " password: " + gardena_password + ".");
	//ju.adapterloginfo(1, "Gardena Smart System Service credentials/2: api key: " + gardena_api_key + " / application secret: " + gardena_application_secret + ".");
	
	incLimitCounter(limitCounter);

	let options_connect_password = {
		url: gardena_authentication_host + URL_OAUTH2_TOKEN,
		method: 'POST',
		json: true,
		headers: {
	...
	let options_connect_client_credentials = {
		url: gardena_authentication_host + URL_OAUTH2_TOKEN,
		method: 'POST',
		json: true,
		headers: {
		...
		form: {
	        ...
	        'client_secret': gardena_application_secret, 
			'client_id': gardena_api_key
		}
	};
	
	let options_connect;
	if (gardena_application_secret !== '' && gardena_api_key !== '') {
		options_connect = options_connect_client_credentials;
		ju.adapterloginfo(1, 'connect ... using client credentials grant');
		ju.consolelog(1, 'connect ... using client credentials grant');
	} else { 
		if (gardena_username !== '' && gardena_password !== '' && gardena_api_key !== '') {
			options_connect = options_connect_password;
			ju.adapterloginfo(1, 'connect ... using password grant');
			ju.consolelog(1, 'connect ... using password grant');
		} else {
			e = new Error('no possible grant for connect');
			if (callback) callback(e);
		}
	}
	...
	request(options_connect, function(err, response, body){
		if (err || !response || response.statusCode >= 300) {
			...
			let timeout = Math.round(response.body.expires_in * configGardenaAuthtokenFactor);
			refresh_timer = setTimeout(reconnect, timeout * 1000);
			ju.adapterloginfo(1, 'Connection: expires_in/timeout/factor:' + response.body.expires_in + ' / ' + timeout + ' / ' + configGardenaAuthtokenFactor);
			...
			if (response.hasOwnProperty('body')) {
				...
				if (parsed_locations.hasOwnProperty('data')) {
					if (parsed_locations.data.length >= 1) {
						...
						// "{"data":[{"id":"139c1da4-cc23-9876-839a-b1ac654909cf","type":"LOCATION","attributes":{"name":"My Garden"}}]}"
...
function get_websocket(callback) {
	...
	request(options_get_websocket, function(err, response, body) {
		...
		//"{"data":{"type":"WEBSOCKET","attributes":{"locationId":"12345678-cd2a-9876-759a-b16c124347cf"},"id":"does-not-matter"}}"
		if (err || !response || response.statusCode >= 300) {
			// no connection or auth failure
...
exports.stopAllTimer = function() {
	ju.adapterloginfo(3, 'stopAllTimer');
	if (refresh_timer !== null) {
		clearTimeout(refresh_timer);
		refresh_timer = null;
	}
